...
void calc_wt_tat(vector<Process> &processes, float &avg_tat, 
float &avg_wt, float &avg_res_time, float &scheduling_len, float &throughput)
{
...
void HRRN(vector<Process> &processes)
{
    ...
    vector<pair<Process, int>> ready_queue;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            if (time >= processes[j].arrival_time && processes[j].completion_time == -1)
            {
                processes[j].waiting_time = time - processes[j].arrival_time;
                float ratio = ((float)(processes[j].waiting_time + processes[j].burst_time)) / (processes[j].burst_time);
                if (hrrt == -1 || (hrrt < ratio))
                {
                    hrrt = ratio;
                    current_process = j;
                }
            }
            else if (processes[j].arrival_time > time)
            {
                if (next > processes[j].arrival_time)
                {
                    next = processes[j].arrival_time;
                    p = j;
                }
            }
        }
        ...
        {
            gantt_chart.push_back({processes[current_process].p_id, time});
        ...
        {
            time = processes[p].arrival_time;
            processes[p].response_time = time - processes[p].arrival_time;
            gantt_chart.push_back({processes[p].p_id, time});
