...
public:
    ...
    int waiting_time;
    int response_time;
    Process() : p_id(-1), arrival_time(-1), burst_time(-1), priority(-1), completion_time(-1), turn_around_time(-1), waiting_time(-1), response_time(-1) {}
    Process(int id, int at, int bt, int pt) : p_id(id), arrival_time(at), burst_time(bt), priority(pt), completion_time(-1), turn_around_time(-1), waiting_time(-1), response_time(-1) {}
};
class compare
{
public:
...
public:
    ...
    {
        if (below.first.priority < above.first.priority || (below.first.priority == above.first.priority && (below.first.arrival_time > above.first.arrival_time || (below.first.arrival_time == above.first.arrival_time && (below.first.p_id > above.first.p_id)))))
            return true;
        return false;
    }
};
...
void priority_scheduling(vector<Process> &processes)
{
    ...
    while (!ready_queue.empty())
    {
        ...
        processes[position].response_time = time;
        time += processes[position].burst_time;
